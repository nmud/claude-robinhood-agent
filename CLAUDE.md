...
- **Intent-then-confirm:** append an INTENT line to positions.md BEFORE placing any order; append CONFIRM (fill price) or CANCELLED after. An INTENT without CONFIRM found at the start of a pass = a possibly-crashed order — resolve against broker open orders/fills before anything else.
- **positions.md is the live ledger:** any entry/exit/stop change updates it in the same pass. Position review reads positions.md, not memory, not findings archaeology.
- **Correlation bucket:** AAPL/MSFT/NVDA/QQQ are one mega-tech bucket. Combined satellite exposure to the bucket ≤ cap in config.md — eight "different" positions in one factor is one big bet wearing eight hats.
